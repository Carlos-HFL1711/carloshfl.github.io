...
$dbConfig = array(
    'host'   => 'localhost',
    'dbname' => 'ptdb_sus', // Nome do banco de dados que será criado
...
function setupDatabase($config) {
    try {
        // 1. Conecta ao MySQL (sem um DB específico) para criar o banco de dados
        $pdo_init = new PDO("mysql:host={$config['host']}", $config['user'], $config['pass']);
        $pdo_init->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $pdo_init->exec("CREATE DATABASE IF NOT EXISTS {$config['dbname']} CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;");

        // 2. Agora, conecta ao banco de dados específico que acabamos de criar/garantir que existe
        $pdo = new PDO("mysql:host={$config['host']};dbname={$config['dbname']}", $config['user'], $config['pass']);
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        // 3. Cria a tabela de pacientes (fila)
        $sql_pacientes = "
        CREATE TABLE IF NOT EXISTS pacientes (
            id INT AUTO_INCREMENT PRIMARY KEY,
            nome VARCHAR(255) NOT NULL,
            cpf VARCHAR(11) NOT NULL, -- Armazena CPF sem pontos ou traços (11 dígitos)
            ...
            posicao_fila INT,
            observacoes TEXT,
            UNIQUE KEY uk_cpf_procedimento (cpf, procedimento) -- Garante que um paciente não entre na mesma fila duas vezes
        );
        ";
        $pdo->exec($sql_pacientes);

        // 4. Cria a tabela para solicitações de revisão
        $sql_revisoes = "
        CREATE TABLE IF NOT EXISTS solicitacoes_revisao (
            id INT AUTO_INCREMENT PRIMARY KEY,
            nome VARCHAR(255) NOT NULL,
            cpf VARCHAR(11) NOT NULL, -- Armazena CPF sem pontos ou traços
        ...
        ";
        $pdo->exec($sql_revisoes);

        // 5. Insere dados de exemplo para teste (apenas uma vez)
        // LÓGICA DE SEEDING MELHORADA: Verifica se o primeiro usuário já existe.
        // Isso previne o erro de 'Duplicate entry' e é mais robusto que a tabela db_init.
        $check_stmt = $pdo->prepare("SELECT COUNT(*) FROM pacientes WHERE cpf = '12345678901'");
        $check_stmt->execute();
        $is_seeded = $check_stmt->fetchColumn();

        if ($is_seeded == 0) {
            // Agora, insere os 15 usuários
            $insert_stmt = $pdo->prepare("
                INSERT INTO pacientes (nome, cpf, data_entrada, olho, procedimento, data_atualizacao, posicao_fila, observacoes)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?);
            ");


        } // Fim do IF $is_seeded == 0


    } catch (PDOException $e) {
        // Se a configuração falhar, exibe um erro fatal.
        die("Erro fatal ao configurar o banco de dados: " . $e->getMessage());
    }
}

// --- ROTEADOR DE REQUISIÇÕES AJAX (BACKEND) ---
// Verifica se a requisição é um POST (enviada pelo JavaScript)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Conecta ao banco de dados que já foi configurado
        ...
        if (isset($input['action'])) {
            ...
            if ($input['action'] === 'search') {
                ...
                // SEGURANÇA (SQL INJECTION): Usa Prepared Statements
                // Busca pelo CPF exato e por um nome 'parcial' (LIKE)
                $stmt = $pdo->prepare("SELECT * FROM pacientes WHERE cpf = ? AND nome LIKE ?");
                ...
                // SEGURANÇA (LGPD): Mascara o CPF antes de enviar de volta ao frontend
                $masked_results = array();
                foreach ($results as $row) {
                    if (!empty($row['cpf']) && strlen($row['cpf']) == 11) {
                        // Formato: 123******01
                ...
                exit; // Termina o script PHP aqui, não renderiza o HTML
            }
            ...
            if ($input['action'] === 'review') {
                ...
                $stmt = $pdo->prepare("
                    ...
                    (nome, cpf, data_nascimento, telefone, whatsapp, observacao, data_solicitacao) 
                    VALUES (?, ?, ?, ?, ?, ?, NOW())
                ");
                ...
                $stmt->execute(array(
                    ...
                    $cpf_limpo,
                    $input['data_nasc'],
                    $telefone_limpo, // Salva o telefone limpo
                    $whatsapp_limpo, // Salva o whatsapp limpo
                    $input['observacao']
                ));
...
// Se não for um POST, o script continua e executa a configuração do DB
// e depois renderiza o HTML abaixo.
setupDatabase($dbConfig);

// O script PHP "termina" aqui e o HTML começa.
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <style type="text/css">
        ...
        * {
        ...
        /* Em telas menores que 850px, esconde a tabela e mostra os cards */
...
<body>
    ...
    <main class="container">
        ...
        <!-- Seção Principal (Busca ou Resultados) -->
        <div id="main-content" class="main-card">
             <div>
                <img class="logo_hoftalon" id="logo_hoftalon" src="https://hoftalon.com.br/wp-content/uploads/2022/10/hoftalon-logo@2x-300x147.png" style="width: 250px;" alt="Hospital Hoftalon" class="header-logo img-fluid" data-pagespeed-url-hash="1100721522" onload="pagespeed.CriticalImages.checkImageForCriticality(this);">
            </div>
                ...
                <form id="searchForm">
                    ...
                    <div class="form-group">
                        <label for="cpf" class="form-label">CPF do Paciente</label>
                         <div class="input-group">
                            <!-- Ícone SVG Documento -->
                            <svg class="input-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
    ...
    <!-- ============================================== -->
    <!-- JAVASCRIPT (FRONTEND)                          -->
    <!-- (O JavaScript é o mesmo, pois ele usa IDs)     -->
    <!-- ============================================== -->
    <script>
        ...
        searchForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                showToast(`Erro no servidor. Verifique seus dados ou solicite uma revisão.`, 'error');
                displayNoResults(); // <-- ESTA É A CORREÇÃO
        ...
        function displayResults(data) {
            ...
            data.forEach(row => {
                // 1. Constrói a linha da TABELA (Desktop)
                tableHTML += `
                    <tr>
                        <td>${escapeHTML(row.id)}</td>
                        <td>${formatarData(row.data_entrada)}</td>
                        <td class="col-nome">${escapeHTML(row.nome)}</td>
                        <td class="col-cpf">${escapeHTML(row.cpf_mask)}</td>
                        ...
                        <td class="col-obs" title="${escapeHTML(row.observacoes)}">${escapeHTML(row.observacoes) || 'N/A'}</td>
                    </tr>
                `;
                
                // 2. Constrói o CARD (Mobile)
                listHTML += `
        ...
        function formatarData(dataISO) {
            ...
            try {
                ...
                if (isNaN(data.getTime())) return dataISO; // Retorna original se inválida
                
                const options = {
                    year: 'numeric',
                    month: '2-digit',
                    day: '2-digit',
                    timeZone: 'UTC' // Importante para datas do DB
                };
                
                // Verifica se a data tem parte de hora relevante
